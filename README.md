...
Backend API base URL is chosen via `NEXT_PUBLIC_ENV` (see below). Contract addresses (TournamentRegistry, pack opener) are in `lib/blockchain.ts`: отдельные константы для production и для UAT при `NEXT_PUBLIC_ENV=development`.
